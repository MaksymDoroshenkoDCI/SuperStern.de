    ...
    <div className="flex flex-col min-h-screen">
      ...
      <section className="bg-slate-900 text-white pt-32 pb-16 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10 space-y-3">
          <h1 className="text-4xl font-extrabold tracking-tight">Unsere Leistungen</h1>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
          {listItems.map((item, idx) => (
            <div
              key={idx}
              className="border border-slate-100 rounded-3xl p-8 shadow-sm hover:shadow-md transition-shadow grid grid-cols-1 md:grid-cols-12 gap-8 items-start"
            >
              {/* Service Intro */}
              <div className="md:col-span-6 space-y-4">
                <div className="bg-amber-100 p-3.5 rounded-2xl inline-block">
                  {item.icon}
                </div>
                <div>
                  <h2 className="text-2xl font-bold text-slate-900">{item.title}</h2>
                  <p className="text-sm font-semibold text-amber-600 mt-1">{item.subtitle}</p>
                </div>
                <p className="text-slate-500 text-sm leading-relaxed">{item.desc}</p>
                <div className="pt-2">
                  <Link
                    href="/preise"
                    className="gold-button text-white font-bold py-2.5 px-6 rounded-full text-xs inline-flex items-center space-x-2"
                  >
              ...
              {/* Checklist */}
              <div className="md:col-span-6 bg-slate-50 rounded-2xl p-6 border border-slate-100">
                <h3 className="font-bold text-slate-800 text-sm mb-4">Was ist im Service enthalten?</h3>
                <ul className="space-y-3">
                  {item.tasks.map((task, index) => (
                    <li key={index} className="flex items-start space-x-2.5 text-xs text-slate-600">
