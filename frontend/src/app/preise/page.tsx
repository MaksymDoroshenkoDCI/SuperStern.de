    ...
    <div className="flex flex-col min-h-screen">
      ...
      <section className="bg-slate-900 text-white pt-32 pb-16 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10 space-y-3">
          <h1 className="text-4xl font-extrabold tracking-tight">Preise & Kalkulator</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-16">
            {priceGrid.map((item, idx) => (
              <div
                key={idx}
                className="border border-slate-200 rounded-3xl p-6 flex flex-col justify-between hover:border-amber-500 hover:shadow-lg transition-all duration-300 bg-slate-50/50"
              >
                <div>
                  <h3 className="font-bold text-slate-800 text-lg mb-2">{item.title}</h3>
                  <div className="mb-4">
                    <span className="text-2xl font-black text-amber-600">{item.rate}</span>
                    <span className="text-xs text-slate-400 block mt-0.5">{item.min}</span>
                  </div>
          ...
          <div className="max-w-3xl mx-auto bg-amber-50 border border-amber-250 p-6 rounded-3xl mb-20 flex flex-col sm:flex-row items-center gap-6">
            <div className="bg-amber-500 p-3.5 rounded-2xl text-white">
              <Percent className="h-8 w-8" />
            </div>
            <div>
              <h4 className="font-bold text-amber-900 text-lg">Steuerlich absetzbar!</h4>
